...
```{r}
# Make column Device
# Group by Device, weekday and Productivity. Summarize Duration with sum
RT_by_weekday <- RT_hourly %>%
  rename(weekday = day_name, Duration = Seconds) %>%
  group_by(Device, weekday, Productivity) %>%
  summarize(Duration = sum(Duration))

# Group by Device and weekday. Summarize Duration with sum
```
```{r}
