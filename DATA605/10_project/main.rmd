...
RT_by_category <- RT_hourly %>%
  ...
  mutate(Productivity = case_when(
    ...
    Productivity == -2 ~ "Very Distracting"),
         ...
         Total_Duartion = sum(Duration)
  )

nhours <- length_days * sum(RT_by_category$Duration) / 60
```
...
make.donut <- function(df, category){
  ...
  text(0, -1, paste0("\n(", round(unique(subset$Total_Duartion)), " min/day)"),
...
walk(levels(RT_by_category$Category), function(Category){
